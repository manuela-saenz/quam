...
// Cargar la orden por ID de transacción
$order = wc_get_order($referenceCode);
WC()->cart->empty_cart();
if ($transactionState == 6 && $polResponseCode == 5) {
	$estadoTx = "Transacci&oacute;n fallida";
	$order->update_status('failed', 'Transacción rechazada por PayU');
	...
	$order->update_status('pending', 'Transacción pendiente por PayU');
